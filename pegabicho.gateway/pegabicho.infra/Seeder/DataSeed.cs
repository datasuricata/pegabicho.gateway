            ...
            using (var context = new AppDbContext(service.GetRequiredService<DbContextOptions<AppDbContext>>())) {
                if (!context.Users.Any()) {
                    ...
                    var root1 = User.Seeder("lucas.moraes@datasuricata.br", "m0raes@123!!",
                        ...
                        new List<Pet> { new Pet("Morgana", null, null, PetSize.Small, PetSpecie.Cat, new DateTime(2015, 09, 22), 2.3f, new Race("Sem raça definida")) },
                        new List<Wallet> { new Wallet(PaymentType.Credit, "0000", "0000", "09319765960", 15, true) },
                        new List<Access> { new Access(UserType.Root) },
                        new List<Document> { new Document("09319765960", null, DocumentType.CPF), new Document("5243516", null, DocumentType.RG) });
                    ...
                    context.Users.AddRange(root1, root2);
                }
